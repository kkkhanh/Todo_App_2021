...
function Average() {
    ...
    const onChange = useCallback(e => {
        console.log('number 혹은 처음 랜더링 될 때만 함수 생성..')
    ...
    },[number,lists]); // nummber 혹은 list가 바뀌었을 때만 함수 생성
    ...
    return (
        <div>
            <input value={number} onChange={onChange} ref={inputEl}/*id대신 사용*/ />
            <button onClick={onInsert}>등록</button>
            <ul>
                {lists/*배열*/.map((list, index) => (
